...
int main(int argc, char *argv[])
{
    ...
    MimeMessage message;
    message.setSender(EmailAddress(QLatin1String("your_email_address@gmail.com"), QLatin1String("Your Name")));
    message.addTo(EmailAddress(QLatin1String("Recipient's Name <recipient@host.com>")));
    ...
    QObject::connect(reply, &ServerReply::finished, [reply] {
        qDebug() << "ServerReply finished" << reply->error() << reply->responseText();
        reply->deleteLater();// Don't forget to delete it

        qApp->quit();  // Поставить флаг что всё послано!
    });
